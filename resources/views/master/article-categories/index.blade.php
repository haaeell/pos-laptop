...
@section('content')
    <div class="mx-auto bg-white rounded-xl">
        ...
        <div class="flex justify-between items-start mb-4">
            <div>
                <h1 class="text-2xl font-semibold text-slate-800">Kategori Artikel</h1>
        ...
        <div class="bg-white rounded-xl shadow border overflow-x-auto px-4 py-5">
            <table class="w-full text-sm">
                ...
                <tbody>
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="p-6 text-center text-slate-400">Belum ada kategori.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <div id="categoryModal" class="fixed inset-0 hidden bg-black/40 backdrop-blur-sm flex items-center justify-center z-50">
        <div class="bg-white w-full max-w-md rounded-2xl shadow-xl">
            ...
            <form id="categoryForm" method="POST" class="p-6 space-y-4">
                ...
                <div>
                    <label class="text-xs font-bold text-slate-600 uppercase tracking-wide">Nama Kategori</label>
        ...
        <script>
            $(document).ready(function () {
                ...
                window.openCreateModal = function () {
                    ...
                    form.attr('action', '{{ route('article-categories.store') }}')
                    method.val('')
                    name.val('')
                }
                ...
                window.openEditModal = function (data) {
                    ...
                    form.attr('action', `/article-categories/${data.id}`)
                    method.val('PUT')
                    name.val(data.name)
                }
                ...
                window.deleteCategory = function (id) {
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            ...
                            form.method = 'POST'
                            form.action = `/article-categories/${id}`
                            form.innerHTML = `
